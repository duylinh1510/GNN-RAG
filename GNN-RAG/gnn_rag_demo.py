...
class GNNRetriever:
    ...
    def retrieve_relevant_nodes(self, query: str, k: int = 5) -> List[str]:
        ...
        # Project query embedding to same dimension as GNN embeddings
        # Simple linear projection
        query_dim = query_tensor.shape[0]
        gnn_dim = self.node_embeddings.shape[1]
        
        if query_dim != gnn_dim:
            # Create a simple projection matrix if not exists
            if not hasattr(self, 'projection'):
                self.projection = torch.randn(query_dim, gnn_dim) * 0.01
            query_tensor = torch.matmul(query_tensor, self.projection)
...
"""
    ...
    def print_prompt(self, question: str) -> str:
        """In ra prompt sẽ được gửi tới LLM để debug"""
    ...
    def generate_cypher(self, question: str, debug: bool = False) -> Tuple[str, List[str]]:
...
def main():
    ...
    try:
        ...
        # Debug options
        debug_mode = st.checkbox("🔍 Debug mode (show full prompt)", value=False)
        ...
        if generate_btn and question:
            with st.spinner("Generating Cypher query using GNN-RAG..."):
                try:
                    ...
                    # Show prompt if debug mode or button clicked
                    if st.button("🔍 Show Full Prompt"):
                        with st.expander("Full Prompt Sent to LLM", expanded=True):
                            prompt = rag_system.print_prompt(question)
